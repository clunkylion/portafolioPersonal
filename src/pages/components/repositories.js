  ...
  useEffect(() => {
    ...
    async function fetchRepos() {
      const response = await fetch(
        "https://api.github.com/users/clunkylion/repos"
      )
  ...
  return (
    <div className="max-w-4xl mx-auto mt-12" data-aos="fade-up">
      <header className="text-center">
        <h2 className="text-3xl font-bold text-blue-700 truncate">
          ...
          return (
            <li className="flex items-center" key={repo.id}>
              <div className="w-9/12 overflow-x-hidden my-4">
                <h4 className="text-blue-700 font-bold">{repo.name}</h4>
                <p
                  className="text-sm overflow-y-hidden text-green-600"
                  style={{ height: "1.5em" }}
                >
                  {repo.description}
                </p>
      ...
      <div className="mt-8 text-center">
        <a
          href="https://github.com/clunkylion/"
          className="btn"
          target="_blank"
          rel="noopener noreferrer"
        >
