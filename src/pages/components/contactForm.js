...
export default () => {
  var hoy = new Date()
  var actual =
    hoy.getDay() + "-" + (hoy.getMonth() + 1) + "-" + hoy.getFullYear()
  ...
  async function sentData() {
    ...
    await axios
      .post("https://portafolio-mongo-api.herokuapp.com/contacto", {
        nombre: name,
        mail: mail,
        fecha: actual,
        idea: contactContent,
  ...
  return (
    <Fragment>
      ...
      <p className="formErrors">{errors?.mail?.message}</p>
      <p className="formErrors">{errors?.contactContent?.message}</p>
    </Fragment>
  )
}
